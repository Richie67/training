...
def parse_args():
    ...
    parser.add_argument("-s", "--sleep",
        type=float, default=0, help="Time in seconds between each request")
...
if __name__ == "__main__":
    ...
    for i in range(args.requests):
        text = make_request(url+"/pay/"+str(i)).strip()
        print(text)
        requests += 1
        if text.startswith("[payment-v1] OK"):
            counters["[payment-v1] OK"] += 1
        elif text.startswith("[payment-v2] OK"):
            counters["[payment-v2] OK"] += 1
        else:
            errors += 1
